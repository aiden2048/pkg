...
// 		logs.Trace("Recv %s to reload config", msg.Subject)
// 		_func(msg.Data)
// 	})
// 	logs.Trace("Listen Config key: %s %+v", config_key, err)
// }
...
func ListenConfigEx(c string, config_key string, _func func([]byte)) {
	...
	err := DoRegistNatsHandler(gNatsconn, subj, queue, func(nConn *nats.Conn, msg *nats.Msg) int32 {
		logs.Trace("Recv_Config %s to reload config:%s", msg.Subject, string(msg.Data))
		_func(msg.Data)
		return 0
	}, 1, onRegSubject)
